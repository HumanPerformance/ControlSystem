...
class data_acquisition( object ):
    ...
    def __init__( self, args, devices_name, devices_BT, smartholder_USB, timeStamp, paramN=None ):
        ...
        self.smartholder_SHH    = smartholder_USB["SHH"]                                    # Store smartholder USB object
...
# ----------------------------------------------------------------------------------------- #
    ...
    def setup( self, args ):
        ...
        self.bpc_flag_new       = 0                                                         # Whether device is in/out of holder
        self.bpc_flag_old       = 0                                                         # Previous state of ^
...
try:
    USB  = createUSBPort( port, baud, timeout )                                            # Create USB connection
except:
    USB  = createACMPort( port, baud, timeout )
finally:
...
for device_ID, device_BT_Obj in smartdevice.iteritems():
    if( device_ID == "stethoscope" ):
##        setToIdle( device_BT_Obj )            # I AM LEAVING A NOTE TO
        statusEnquiry( device_BT_Obj )          # GET THIS FIXED
    else:
